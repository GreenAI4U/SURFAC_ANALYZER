...
const Settings = () => {
    const [activeTab, setActiveTab] = useState("Camera Setting");
    const Tabs = ["All Setting", "Camera Setting", "Paths for saving", "Saving to a directory", "Upscaling", "Face restoration"];

    return (
        <div className="bg-primary-950 min-h-screen">
            <div className="px-4 py-6">
                <div className="flex gap-3 justify-between">
                    <div className="w-56 space-y-2 bg-primary-850 p-3 rounded-lg text-white h-[calc(100vh-9rem)] sticky top-4">
                        {Tabs.map((item, index) => (
                            <button
                                key={index}
                                className={"w-full p-1 px-2 text-left hover:bg-primary-500 hover:text-black hover:font-semibold rounded transition-all " + (activeTab === item ? "bg-primary-200 text-black font-semibold" : "")}
                                onClick={() => {
                                    setActiveTab(item);
                                }}
                            >
                    ...
                    <div className="flex-1 rounded-lg bg-primary-850 p-4">
                        {(activeTab === "All Setting" || activeTab === "Camera Setting") && (
                            <div className="flex flex-col space-y-4">
                                <div className="flex  gap-10">
                                    <div className="flex flex-col gap-1">
                                        <label className="text-white">Width of Image (px)</label>
                                        <input defaultValue={4096} className="w-full px-3 rounded-lg py-1.5 mt-1.5 bg-white text-black placeholder:text-black outline-none focus:outline-none" type="number" />
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <label className="text-white">Height of Image (px)</label>
                                        <input defaultValue={2176} className="w-full px-3 rounded-lg py-1.5 mt-1.5 bg-white text-black placeholder:text-black outline-none focus:outline-none" type="number" />
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <label className="text-white">Left Margin (cm)</label>
                                        <input defaultValue={30} className="w-full px-3 rounded-lg py-1.5 mt-1.5 bg-white text-black placeholder:text-black outline-none focus:outline-none" type="number" />
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <label className="text-white">Right Margin (cm)</label>
                                        <input defaultValue={25} className="w-full px-3 rounded-lg py-1.5 mt-1.5 bg-white text-black placeholder:text-black outline-none focus:outline-none" type="number" />
                                    </div>
                                    <div className="flex flex-col gap-1">
                                        <label className="text-white">Total Distance (cm)</label>
                                        <input defaultValue={190} className="w-full px-3 rounded-lg py-1.5 mt-1.5 bg-white text-black placeholder:text-black outline-none focus:outline-none" type="number" />
                                    </div>
                                </div>
                        ...
                        {(activeTab === "All Setting" || activeTab === "Paths for saving") && <div className="flex flex-col space-y-4"></div>}
                        {(activeTab === "All Setting" || activeTab === "Saving to a directory") && <div className="flex flex-col space-y-4"></div>}
                        {(activeTab === "All Setting" || activeTab === "Upscaling") && <div className="flex flex-col space-y-4"></div>}
                        {(activeTab === "All Setting" || activeTab === "Face restoration") && <div className="flex flex-col space-y-4"></div>}
                        <button className="w-fit bg-primary-950 text-white rounded-lg text-center p-2 px-5 mt-5 cursor-pointer">Apply setting</button>
                    </div>
                </div>
            </div>
        </div>
