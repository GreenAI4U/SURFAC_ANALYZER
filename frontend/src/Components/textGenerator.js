...
const TextGenerator = ({ text, countArr, setCountArr }) => {
    ...
    async function fetchWord() {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            // fetch("https://www.dictionary.com/browse/" + word, requestOptions)
            ...
            // 		} catch {
            // 			fetch("https://www.vocabulary.com/dictionary/definition.ajax?search=" + word + "&lang=en", requestOptions)
            // 				.then((response) => response.text())
            // 				.then((result) => {
            // 					try {
            ...
            // 	});
            try {
                const response = await fetch("https://www.dictionary.com/browse/" + word, requestOptions);
                const result = await response.text();

                try {
                    let partOfSpeech = result
                        .split('class="luna-pos">')[1]
                        .split("</")[0]
                        .replace(/,/g, "");
                    let classes = classDict[partOfSpeech.charAt(0).toUpperCase() + partOfSpeech.substr(1).toLowerCase()];
                    words[i].classList.add(classes.split(" ")[0], classes.split(" ")[1]);
                    ...
                    const response = await fetch("https://www.vocabulary.com/dictionary/definition.ajax?search=" + word + "&lang=en", requestOptions);
                    const result = await response.text();
                    try {
                        let partOfSpeech = result
                            .split('title="')[1]
